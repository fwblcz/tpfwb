...
class User extends AuthController
{
    ...
    {
        if($id) $userinfo=UserModel::get($id);
        $field[]= Form::input('nickname','会员名称',isset($userinfo) ? $userinfo->nickname : '')->col(Form::col(24));
        $field[]= Form::input('real_name','会员真实姓名',isset($userinfo) ? $userinfo->real_name : '')->col(Form::col(24));
        $field[]= Form::input('phone','手机号',isset($userinfo) ? $userinfo->phone : '')->col(Form::col(24));
        $field[]= Form::formFrameImageOne('avatar','头像',isset($userinfo) ? $userinfo->avatar : '');
        $field[]= Form::date('birthday','生日');
        $field[]= Form::radio('sex','性别',isset($userinfo) ? $userinfo->sex : 0)->options([['label'=>'男','value'=>1],['label'=>'女','value'=>2],['label'=>'保密','value'=>0]])->col(8);
        $field[]= Form::input('pwd','密码',isset($userinfo) ? $userinfo->pwd : '')->type('password');
        $field[]= Form::input('conf_pwd','确认密码')->type('password');
        $field[]= Form::radio('status','状态',1)->options([['label'=>'正常','value'=>1],['label'=>'锁定','value'=>0]]);
        $form = Form::make_post_form('添加会员',$field,Url::build('save',['uid'=>$id]),2);
    ...
    public function save(Request $request)
    {
        ...
        if(!$data['nickname']) return Json::fail('请输入会员名称');
        if(!$data['phone']) return Json::fail('请输入手机号');
        if(!$data['pwd']) return Json::fail('请输入密码');
        if($data['pwd'] != $data['conf_pwd']) return Json::fail('两次输入密码不想同');
        if(UserModel::be($data['nickname'],'nickname')) return Json::fail('管理员账号已存在');
        $data['pwd'] = md5($data['pwd']);
        unset($data['conf_pwd']);
        UserModel::set($data);
        return Json::successful('添加会员成功!');
    }
